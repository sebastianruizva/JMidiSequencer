...
public class JMidiTrack {
   ...
   *                                  event.
   */
  protected void editEvent(JMidiEvent event, int newPitch, int newTick, int newDuration, int newVelocity, int newChannel) throws IllegalArgumentException {
    ...
    //Create new event
    JMidiEvent newEvent = JMidiEvent.builder().pitch(newPitch).tick(newTick).duration(newDuration).velocity(newVelocity).channel(newChannel).build();
  ...
  protected void moveEvent(JMidiEvent event, int newPitch, int newTick)
    ...
    this.editEvent(event, newPitch, newTick, event.getDuration(), event.getVelocity(), event.getChannel());
  ...
  protected void resizeEvent(JMidiEvent event, int newDistance)
    ...
    this.editEvent(event, event.getPitch(), event.getTick(), newDistance, event.getVelocity(), event.getChannel());
